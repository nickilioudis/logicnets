...
def total_loss(teacher_probs, student_probs, student_preds, labels):
      distillation_criterion = nn.KLDivLoss()
      alpha=0.1
      # temperature=10
      student_loss_m = student_loss(student_preds, labels) # gave _m subscript since no longer have class and would confuse student_loss var with function
      distillation_loss = (
...
def train(model, datasets, train_cfg, options, teacher_model): # N - added teacher model
    ...
    for epoch in range(0, num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            teacher_probs = torch.nn.functional.softmax(teacher_model.forward(data), dim=1) #NOTE: reason why accuracy might be bad is bc distillation loss thinks predictions are PRE-softmax probabilities whereas here they are actual labels??
            teacher_preds = teacher_probs.detach().max(1, keepdim=True)[1]
            teacher_preds = torch.cat(tuple(teacher_preds))
            ...
            loss = total_loss(teacher_probs, output, pred, target)
            ...
            print("teacher_preds:", teacher_preds)
            print("output:", output)
            print("pred:", pred)
            print("target:", target)
...
if __name__ == "__main__":
    ...
    model_cfg['output_length'] = 10 #one-hot vec, 10 diff classes in MNIST # N - changed from len(y)
